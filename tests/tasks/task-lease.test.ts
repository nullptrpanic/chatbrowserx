...
async function seedLeaseTask(
  ...
  const records = createTaskRecords(
    ...
    {
      clock: { now: () => 1_000 },
      ids: { create: (prefix) => `${prefix}_${suffix}_${String(++id)}` },
    },
  );
...
describe('TaskLeaseManager', () => {
  it('allows takeover only after a 30-second lease expires', async () => {
    ...
    const { task, run } = await seedLeaseTask(database, 'takeover');
    ...
    await expect(repository.getRun(run.id)).resolves.toMatchObject({ lease: null });
    database.close();
  });
  ...
  it('renews an owned lease without changing its generation', async () => {
    ...
    const repository = new IndexedDbTaskRepository(database);
    const manager = new TaskLeaseManager(repository);
    const { task, run } = await seedLeaseTask(database, 'renew');
