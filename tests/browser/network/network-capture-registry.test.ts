...
describe('NetworkCaptureRegistry', () => {
  ...
  it('records redirects as separate opaque entries and returns newest filtered summaries', async () => {
    ...
    const entries = await capture.list(7, 'api.test', 10);
  ...
  it('keeps only 500 entries and caps list results at the requested maximum', async () => {
    ...
    const entries = await capture.list(7, '', 100);
  ...
  it('fetches bodies only on demand and redacts headers, query values, and JSON secrets', async () => {
    ...
    const [summary] = await capture.list(7, '', 10);
    if (!summary) throw new Error('Expected captured request.');

    const metadata = await capture.get(7, summary.requestId, false);
    expect(debuggerTransport.send).not.toHaveBeenCalledWith(
      expect.anything(),
      'Network.getResponseBody',
      expect.anything(),
    );
    expect(metadata).toMatchObject({ body: { included: false, available: true } });

    const details = await capture.get(7, summary.requestId, true);
    expect(debuggerTransport.send).toHaveBeenCalledWith({ tabId: 7 }, 'Network.getResponseBody', {
      requestId: 'cdp_body',
    });
    expect(details.requestHeaders).toEqual({ Accept: 'application/json' });
    ...
    expect(details.body.text).toContain('"accessToken":"[redacted]"');
    expect(details.body.text).toContain('"value":"visible"');
  ...
  it('marks binary and unavailable bodies without leaking payloads', async () => {
    ...
    await expect(capture.get(7, summary.requestId, true)).resolves.toMatchObject({
      body: { included: true, available: false, reason: 'binary' },
    });
    debuggerTransport.send.mockRejectedValueOnce(new Error('private CDP failure'));
    await expect(capture.get(7, summary.requestId, true)).resolves.toMatchObject({
      body: { included: true, available: false, reason: 'unavailable' },
    });
  });
  ...
  it('invalidates opaque request IDs on stop and root debugger detach', async () => {
    ...
    await expect(capture.get(7, summary.requestId, false)).rejects.toBeInstanceOf(
      NetworkCaptureError,
    );
    await expect(capture.list(7, '', 10)).rejects.toMatchObject({
