...
export type ImageBatchValidation<T = File> =
  | { readonly ok: true; readonly files: readonly T[] }
...
export function validateImageBatch<T extends Pick<Blob, 'size' | 'type'>>(
  files: readonly T[],
  existing: ExistingImageUsage = { count: 0, bytes: 0 },
): ImageBatchValidation<T> {
