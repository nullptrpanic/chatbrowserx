...
import { inspectHttpFailure, isAbortFailure } from '../../shared/net/http-failure';
import { isPublicHttpUrl } from '../../shared/net/public-http-url';
import { isTavilyError, tavilyErrorFromCode } from './error';
import {
...
async function throwTavilyHttpError(response: Response): Promise<never> {
  const failure = await inspectHttpFailure(response);
  if (failure.status === 401 || failure.status === 403) {
    throw tavilyErrorFromCode('AUTH', { status: failure.status });
  }
  if (failure.status === 429) {
    throw tavilyErrorFromCode('RATE_LIMIT', failure);
  }
  if (failure.status >= 500) {
    throw tavilyErrorFromCode('TRANSIENT', failure);
  }
  throw tavilyErrorFromCode('INVALID_RESPONSE', { status: failure.status });
}
...
async function readBoundedJson(response: Response, signal: AbortSignal): Promise<unknown> {
  ...
  if (!contentType.includes('application/json') || response.body === null) {
    ...
    throw tavilyErrorFromCode('INVALID_RESPONSE', { status: response.status });
  }
  ...
  try {
    while (true) {
      if (signal.aborted) throw tavilyErrorFromCode('ABORTED');
      const next = await reader.read();
      if (next.done) break;
      bytes += next.value.byteLength;
      if (bytes > MAX_RESPONSE_BYTES) {
        await reader.cancel().catch(() => undefined);
        throw tavilyErrorFromCode('INVALID_RESPONSE', {
    ...
    if (isTavilyError(error)) throw error;
    if (signal.aborted || isAbortFailure(error)) throw tavilyErrorFromCode('ABORTED');
    throw tavilyErrorFromCode('TRANSIENT');
  } finally {
    reader.releaseLock();
  }
...
function normalizeResults(
  ...
  for (const value of values.slice(0, maxResults)) {
    if (!isPublicHttpUrl(value.url)) throw tavilyErrorFromCode('INVALID_RESPONSE');
    if (remaining === 0) {
      truncated = true;
      break;
    }
...
function parseSearchResults(value: unknown, maxResults: number): TavilyResultSet {
  ...
  if (!envelope.success) throw tavilyErrorFromCode('INVALID_RESPONSE');
  const results = envelope.data.results.map((item) => {
    const parsed = searchResultSchema.safeParse(item);
    if (!parsed.success) throw tavilyErrorFromCode('INVALID_RESPONSE');
    return {
...
export class TavilyClient implements TavilyExecutionPort {
  ...
  async isConfigured(): Promise<boolean> {
    try {
      return Boolean((await this.#credentials.getTavilyKey())?.trim());
    } catch {
      return false;
    }
  }
  ...
  async #request(
    ...
    try {
      ...
      throw tavilyErrorFromCode('AUTH');
    }
    if (!key || key.trim().length === 0) throw tavilyErrorFromCode('AUTH');
