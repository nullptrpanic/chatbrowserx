...
for (const cdn of [false, true])
  extensionTest(
    `translates a ${cdn ? 'cross-origin rounded bordered' : 'local'} static image without hiding the lens or sampling unchanged pixels`,
    async ({ extensionSession }, testInfo) => {
      const { context, sidePanelPage: panel } = extensionSession;
      const token = Buffer.from(
        JSON.stringify({
          'https://api.openai.com/auth': { chatgpt_account_id: 'acct_image' },
        }),
      ).toString('base64url');
      ...
      const png = await panel.evaluate(() => {
        ...
        ctx.fillStyle = 'black';
        ctx.font = '40px Arial';
        ctx.fillText('IMAGE TEXT', 80, 100);
        return canvas.toDataURL();
      });
      const sourceUrl = 'https://translation-cdn.test/rounded.png';
      if (cdn)
      ...
      await context.route('https://translation-static.test/', (route) =>
        route.fulfill({
          ...
          body: `<!doctype html><style>body{margin:0;min-height:2000px;background:#f1f5fa}p{position:absolute;left:400px;top:220px}img{position:absolute;left:400px;top:300px;width:400px;height:200px;${cdn ? 'border:1px solid #ddd;border-radius:8px;' : ''}}img+img{left:1000px}</style><p>Hello text</p><img alt="Test image"><script>
      const img=document.querySelector('img');img.src=${JSON.stringify(cdn ? sourceUrl : png)};img.after(img.cloneNode());
      ...
      // Image origin + border + OCR y, with a one-CSS-pixel mask margin.
      const patchTop = 300 + (cdn ? 1 : 0) + 200 * 0.15 - 1;
      expect(beforeScroll.y).toBeCloseTo(patchTop - 40, 0);
      ...
      expect(afterScroll?.y).toBeCloseTo(patchTop - 80, 0);
      ...
      for (const scroll of [20, 40, 0]) {
        ...
        await expect(lens).toHaveAttribute('data-status', 'ready');
        expect((await imageTextRect())?.y).toBeCloseTo(patchTop - scroll, 0);
